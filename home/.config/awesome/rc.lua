...
local confpath = awesome.conffile:match("(.*)/[^/]*$")
...
function createwidget(options)
    ...
    if options.icon then
        ...
        icon:set_image(confpath .. "/icons/" .. options.icon)
...
awful.screen.connect_for_each_screen(function(s)
    ...
    for index, tag in pairs(s.tags) do
        tag.gap = 2
        tag.gap_single_client = 0
    end
...
local main_master = function (c)
    local count = awful.screen.focused().selected_tag.master_count
    awful.client.swap.byidx(count, c)
end
